...
export const getRecruits = async (): Promise<Recruit[]> => {
  const result = await axios.get("http://localhost:8080/user-recruit");
  return result.data;
};

//募集作成
export const createRecuit = async (
  recruitData: FormRecruitData,
  // userId: any
): Promise<{ message: string; success: boolean }> => {
  try {
    //userIdはstring型？
    const res = await axiosInstance.post("/user-recruit", recruitData);
