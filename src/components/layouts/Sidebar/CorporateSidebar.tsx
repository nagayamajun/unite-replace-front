...
export const CorporateSideBar = (): JSX.Element => {
  ...
  const menuLinks = [
    ...
    {
      href: `/profiles/corporation/${employee?.belongToCorporation.id}`,
      label: "所属企業情報",
      icon: <CiViewList />,
    },
    // {href: '/', label: 'お問合せ'},
      ...
      <div className="flex flex-col items-center">
        {menuLinks.map((e, index) => (
          <Link
            key={index}
            href={e.href}
            className="flex flex-row items-center justify-start font-semibold h-16 ml-4 w-full hover:bg-green-500"
          >
